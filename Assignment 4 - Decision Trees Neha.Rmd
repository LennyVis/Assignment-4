...
```{r,message=FALSE, warning=FALSE}
treefit <- rpart(Type ~ . - Type, data = train, control = rpart.control())
```
...
To assess the performance of the classification tree, we predict on the `test` dataframe using the `predict` fucntion and svaing the output in `out` dataframe.
...
We change the default paramters.
