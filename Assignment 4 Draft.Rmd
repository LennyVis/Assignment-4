---
title: "Assignment 4- MA710 Dataminers"
output:
  html_document:
    toc: true
---
...
There are 21 variables that we are interested in. The variables name,  type and descriptions are listed below. 
...
* Apply k nearest neighbour and decision tree model algorithms for predicting the type of institution
...
The output tells us that the data frame `csb.vars.df` has 6893 observations and 27 variables. It also displays the class of these variables. We can see that some of the variables do not belong to the correct class. For instance, `CONTROL`(Type of institution - Public or Private) should be a factor variable, but it is an integer in the data frame. Similarly FEMALE (Percentage of female students) should be numeric, and not a character variable.
...
The variables `NTP4_PRIV` and `NTP4_PUB` describes the average net price of either Public or Private colleges, and are complementing each other. For this reason, we will build a new variable `NPT4` which describes the average net price for both public and private colleges.
...
To split the dataset into training and test sets, we first set a seed so that we can get the same sequence of random numbers whenever we supply the same seed in the random number generator.
...
### 6.3 Predicting on Test dataset

To assess the performance of the classification tree, we predict on the `test` dataframe using the `predict` function and saving the output in `out` dataframe.
...
The output tells us that each row contains the predcited probabilities for each value of the target variable `Type`. For instance, we can see that predicted probability for the first observation to be classified as a Private college is 0.9651 and to be a Public college is 0.03482. For each observation, we'll need to find which class has the highest probability and then assign that class to the observation.
...
### 6.4 Changing control parameters
...
On changing the parameters, we are able to successfully reduce the misclassification rate from 7.67 % to 6.98 %.
...
As we can see, the `xerror` drops dramatically till the ninth split and then decreases slowly till the twentieth split.
...
### 7.3 Predicting on Test dataset
...
The output tells us that each row contains the predicted probabilities for each value of the target variable `Type`. For instance, we can see that predicted probability for the first observation to be classified as a Private college is 0.97848101 and to be a Public college is 0.02151899. For each observation, we'll need to find which class has the highest probability and then assign that class to the observation.
...
library(e1071)
```
The k-nearest neighbors model is built using the training data set. 
```{r}
...
The prediction is made on test dataset from k-nearest neighbors model. And we could take a look at first ten predictions.
```{r}
kprediction<-predict(kneighbors,test)
head(kprediction, n=10)
```
...
## 9 K-nearest Neighbors - Part II

The `caret` package makes the `confusionMatrix` function
([documentation](https://artax.karlin.mff.cuni.cz/r-help/library/caret/html/confusionMatrix.html) available. The `kknn` package makes the `kknn` function available.
...
We are reading and displaying the first 10 rows from the train set after the variable transformations. We can see that 
```{r}
Data <- csb.ren.df
head(Data, 10)
```
...
We can see that the variables are with different scales. Since we are performing the k-nearest neightbours algorithm, the distance once calculated, will determine the final prediction. If we have variables with different scales, the ones which have larger scale will influence the distance calculation in a stronger way. In order to avoid scale influences on the distance calculation, we will perform scaling of the variables.
...
#colMeans(train)  # faster version of apply(scaled.dat, 2, mean)
#apply(scaled.data, 2, sd)
```

At this point we are ready to run the k-neighbours model and get predictions for the testing data set. We will first perform the knn algorithm by using the default parametrization. We will look into predicting based on 9 maximum number of neighbors.
...
We can see that with this model, the best result with the minimal missclassification rate happens when the maximum number of neighbours (9) is used. We will apply the predict function on the testing data set with the results shown below (show first 10 predictions).
...
```
We have achieved an accuracy of 92%. The sensitivity (or the proportion of positives that are correctly identified) is 88.5% and the specificity (the proportions of negatives that are correctly identified) is 93.8%. 

When plotting the missclassification against the number of the neighbours used we can see that with the increase in the number of neighbours, there is a decrease in the missclassification.
...
The k-neighbors algorithm from the train.kknn package allows us to explore how using different kernels and different values of the kmax parameter affects the results. We are also able to see from the results which values are optimal for achieving the minimal misclassification error.
...
We can see that we get the minimum classification error when kernel = "biweight" and the maximum number of k is equal to 34.
...
#table(testing[,1], prediction)
```
We have achieved an accuracy of 92.9%. The sensitivity (or the proportion of positives that are correctly identified) is 90.1% and the specificity (the proportions of negatives that are correctly identified) is 94.5%. 
...
We can see that in this case we get the minimum classification error when kernel = "biweight" and the maximum number of k is equal to 30 We will check the confusion matrix for this solution.
...
#table(testing[,1], prediction)
```
We have achieved an accuracy of 93.8%. The sensitivity (or the proportion of positives that are correctly identified) is 92.2% and the specificity (the proportions of negatives that are correctly identified) is 94.8%. 
...
We can see that in this case we get the minimum classification error when kernel = "biweight" and the maximum number of k is equal to 25. We will check the confusion matrix for this solution.
...
We can see that in this case we get the minimum classification error when kernel = "biweight" and the maximum number of k is equal to 18. We will check the confusion matrix for this solution.
...
We have achieved an accuracy of 95.4%. The sensitivity (or the proportion of positives that are correctly identified) is 94.4% and the specificity (the proportions of negatives that are correctly identified) is 95.9%.
...
We can see that in this case we get the minimum classification error when kernel = "triangular" and the maximum number of k is equal to 15. We will check the confusion matrix for this solution.
...
We have achieved an accuracy of 94.7%. The sensitivity (or the proportion of positives that are correctly identified) is 93.7% and the specificity (the proportions of negatives that are correctly identified) is 95.2%. 

We can conclude that the best results are achieved when the kernel = "biweight", the maximum number of k is equal to 18 and the Markowski distance is 0.35. the accuracy score is 95.4%.

## 10 Comparison and Conclusion

### 10.1 Optimal model

We would like to compare the best decision tree model and the K-nearest Neighbors model with the highest accuracy score we've built to find an optimal one in our case:

|Model        |Accuracy|
|-----------------|---------|-------------------------------
|Decision Tree|93.6% |
|K-nearest Neighbors | 95.4%|

In our case, the optimal model that gives us the lowest missclassfication rate is the K-nearest Neighbors with the kernel = "biweight", the maximum number of k equals to 18 and the Markowski distance is 0.35. The accuracy score reaches 95.4%. 

### 10.2 Important variables for classification
From the plot of decision tree model, we could define that `Cost` (Average annual cost of attendance a college) is the most essential variable for classifying a college into Public or Private. In addition, `Female_perc` and `Fin_Indep_perc` are also very important variables for classification.

### 10.3 Conclusion
In terms of accuracy score, K-nearest Neighbors model is the optimal model in our case. However,we might choose different models based on the specific situation.

The main advantage of the decision tree model is interpretability. Decision trees are "white boxes" in the sense that the acquired knowledge can be expressed in a readable form, while KNN is generally black box, i.e. you cannot read the acquired knowledge in a comprehensible way. 

In addition, KNN determines neighborhoods, so there must be a distance metric. This implies that all features must be numeric. Distance metrics may be effected by varying scales between attributes and also high-dimensional space. DT on the other hand predicts a class for a given input vector. The attributes may be numeric or nominal.

In conclusion, the decision tree is easy to interpret. However, complexity is the down side and the tree might get too large even after some pruning.

KNN is a black box, and the net model is not interpretable, but the accuracy is usually high in practice. 

As a data analyst, we need to choose the proper model caerfully based on our demand.