...
The prediction is made on test dataset from k-nearet neighbors model. And we could take a look at first ten predictions.
```{r}
...
We would like to compare the best decision tree model and the K-nearest Neighbors model with the highest accuracy score we've built to find a optimal one in our case:
...
In our case, the optimal model that gives us the lowest missclassfication rate is the K-nearest Neighbors with the kernel = "biweight", the maximum number of k is equal to 18 and the Markowski distance is 0.35. The accuracy score reaches 95.4%. 
...
The main advantage of decision tree model is interpretability. Decision trees are "white boxes" in the sense that the acquired knowledge can be expressed in a readable form, while KNN is generally black box, i.e. you cannot read the acquired knowledge in a comprehensible way. 
...
In conclusion, Decision tree is easy to interpret. However, complexity is the down side and the tree might get too large even after some pruning.

KNN is a black box, and the net model is not interpretable, but the accuracy usually high in practice. 
